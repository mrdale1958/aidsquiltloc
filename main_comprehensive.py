...
class AIDSQuiltScraper:
    ...
    async def _scrape_metadata(self, max_items: int = None):
        ...
        while True:
            try:
                # Get batch of items
                self.logger.info("Fetching items batch: start=%d, count=%d", start, batch_size)
                ...
                if not items:
                    self.logger.info("No more items found, scraping complete")
                    break
                ...
                for item in items:
                    ...
                    if max_items and self.stats['items_processed'] >= max_items:
                        self.logger.info("Reached max_items limit (%d)", max_items)
                        return
                ...
                if len(items) < batch_size:
                    self.logger.info("Received fewer items than requested, scraping complete")
                    break
    ...
    async def _process_single_item(self, item_data, item_details=None):
        ...
        try:
            ...
            if not item_id:
                self.logger.warning("Could not extract item ID from: %s", item_data.get('id', 'unknown'))
                return
            ...
            if not item_details:
                try:
                    ...
                    self.logger.warning("Could not get details for item %s: %s", item_id, e)
                    item_details = None
            ...
            try:
                ...
                self.logger.warning("Could not get resources for item %s: %s", item_id, e)
                resources = []
    ...
    async def _download_missing_images(self):
        ...
        self.logger.info("Found %d items without downloaded images", len(items_without_images))
        ...
        for item_id in items_without_images:
            try:
                ...
                self.logger.error("Error downloading images for item %s: %s", item_id, e)
                self.stats['errors'] += 1
